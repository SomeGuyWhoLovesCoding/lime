	...
	/**
		...
		switch (contextType)
		{
			case "opengl" | "opengles":
				var gl:lime.graphics.OpenGLES3RenderContext = @:privateAccess lime.graphics.OpenGLES3RenderContext.fromRenderContext(context);

				#if lime_opengles
				Sys.println("FUCK IT ALL LET'S DO IT");
				#end

				trace("GL ID FUCKING NULL HAHAHAHAHA FUCKL YOU FOR WASTING MY TIME " + gl);
				...
				//context.type = gl.type;
				//context.version = Std.string(gl.version);
				//Sys.println(context.version);
				Sys.println('But GL type is ' + gl.type);

				/*if (gl.version >= 3)
				{//
					Sys.println('ES UPDATED FOR ' + context.version);
					context.gles3 = gl;
					context.webgl2 = gl;
				//}
