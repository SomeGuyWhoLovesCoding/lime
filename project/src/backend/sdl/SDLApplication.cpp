...
namespace lime
{
    ...
    static std::vector<int64_t> predictable_frames;
    static int currentPredictionIndex = 0;
    static bool predictionSystemInitialized = false;
    static int64_t lastTickedUnit = -1;  // ADD THIS

    static void generateFrames(int64_t now10ns) {
        int64_t to_units = now10ns / UPDATE_PERIOD_10NS;
        predictable_frames.clear();
        predictable_frames.reserve(32);
        for (int i = 0; i < 32; i++) {
            predictable_frames.push_back(to_units + i + 1);
        }
        currentPredictionIndex = 0;
        predictionSystemInitialized = true;
    }

    static bool isPastDeadline(int64_t now10ns) {
        int64_t to_units = now10ns / UPDATE_PERIOD_10NS;
        
        // Already ticked this frame unit — don't tick again
        if (to_units == lastTickedUnit) {
            return false;
        }
        
        if (!predictionSystemInitialized || predictable_frames.empty()) {
            generateFrames(now10ns);
            return false;
        }
        
        if (currentPredictionIndex >= (int)predictable_frames.size()) {
            generateFrames(now10ns);
            return false;
        }
        
        bool isAtDeadline = to_units >= predictable_frames[currentPredictionIndex];
        
        if (isAtDeadline) {
            lastTickedUnit = to_units;  // ADD THIS
            currentPredictionIndex++;
            
            if (currentPredictionIndex >= 24) {
                generateFrames(now10ns);
            }
        }
        return isAtDeadline;
    ...
    static void resetPredictionSystem() {
        predictable_frames.clear();
        currentPredictionIndex = 0;
        predictionSystemInitialized = false;
        lastTickedUnit = -1;  // ADD THIS
    }
	...
	void SDLApplication::SetFrameRate(double frameRate)
	{
        ...
        // Reset prediction system when framerate changes
	...
	static void waylandFrameCallbackHandler(void* data, struct wl_callback* callback, uint32_t time) {
		...
		int64_t now = getTime10ns();
		render_timestamp = UPDATE_PERIOD_10NS;
	...
	void handleLinuxVsync(SDL_Window* sdlWindow, int64_t now10ns, int64_t lag, int64_t& nextRenderTime10ns, bool& shouldRender) {
		...
		if (!shouldRender) {
            shouldRender = isPastDeadline(now10ns);
        }
	}
