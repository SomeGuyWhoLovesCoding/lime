...
namespace lime
{
    ...
    // ========================================================================
    // Predictive Frame Time Loop
    // ------------------------------------------------------------------------
    // Replaces the legacy fallback timer-based render scheduling. Instead of
    // computing each frame deadline on the fly, we pre-compute batches
    // ("generations") of 32 predicted frame deadlines spaced `period10ns`
    // apart.
    //
    // Each call to consume() pops one deadline when it is due. When more than
    // POP_THRESHOLD (24) deadlines have been popped from the current
    // generation, a new generation of GENERATION_SIZE (32) is automatically
    // generated and appended to the timeline.
    //
    // Calling setPeriod() — which is invoked from SetFrameRate — also
    // generates a fresh batch of 32, keeping the timeline in sync with the
    // desired frame rate.
    //
    // The ring buffer has capacity for 2 generations (64 slots), which is
    // sufficient because at most 7 unpopped frames remain from the previous
    // generation when a new one is generated (32 - 25 = 7), giving a worst
    // case of 7 + 32 = 39 queued items.
    // ========================================================================
    class PredictiveFrameTimeline {
    public:
        static constexpr size_t GENERATION_SIZE = 32;
        static constexpr size_t POP_THRESHOLD = 24;
        static constexpr size_t CAPACITY = GENERATION_SIZE * 2;

    private:
        std::array<int64_t, CAPACITY> deadlines{};
        size_t head = 0;
        size_t tail = 0;

        // Monotonically increasing counters (never wrap) used to determine
        // which generation a popped frame belongs to.
        uint64_t totalPushed = 0;
        uint64_t totalPopped = 0;
        uint64_t currentGenStartPushed = 0;

        int64_t nextDeadlineBase10ns = 0;
        int64_t period10ns = 0;
        bool initialized = false;

    public:
        /// Update the frame period and immediately generate a fresh batch of
        /// 32 predicted deadlines. Called from SetFrameRate.
        ...
        /// Generate a new generation of GENERATION_SIZE predicted deadlines,
        /// appended to the end of the timeline. Resets the per-generation pop
        /// counter.
        void generateBatch()
        {
            if (period10ns <= 0)
            ...
            if (!initialized)
            {
                nextDeadlineBase10ns = getTime10ns() + period10ns;
                initialized = true;
            }

            currentGenStartPushed = totalPushed;

            for (size_t i = 0; i < GENERATION_SIZE; ++i)
            {
                deadlines[tail] = nextDeadlineBase10ns;
                tail = (tail + 1) % CAPACITY;
                nextDeadlineBase10ns += period10ns;
                totalPushed++;
            }
        }

        /// Attempt to consume the next predicted deadline. Returns true if the
        /// deadline has been reached (i.e., it is time to render), in which
        /// case the deadline is popped from the front of the timeline.
        ///
        /// If more than POP_THRESHOLD deadlines have been popped from the
        /// current generation after this pop, a new generation of
        /// GENERATION_SIZE is generated automatically.
        bool consume(int64_t now10ns)
        {
            if (head == tail)
            {
                // Timeline empty — generate a fresh batch on demand.
                generateBatch();
                if (head == tail)
                    return false;
            }

            if (now10ns >= deadlines[head])
            {
                uint64_t poppedFrameIndex = totalPopped;
                head = (head + 1) % CAPACITY;
                totalPopped++;

                // Only count pops that belong to the current (most recent)
                // generation. Pops of leftover frames from a previous
                // generation do not trigger a new generation.
                if (poppedFrameIndex >= currentGenStartPushed)
                {
                    uint64_t poppedInCurrentGen =
                        totalPopped - currentGenStartPushed;
                    if (poppedInCurrentGen > POP_THRESHOLD)
                    {
                        generateBatch();
                    }
                }
        ...
        /// Clear the timeline completely. Used when the frame rate is set to 0.
        void reset()
        {
            head = tail = 0;
            totalPushed = 0;
            totalPopped = 0;
            currentGenStartPushed = 0;
            initialized = false;
        }
    ...
    static PredictiveFrameTimeline predictiveTimeline;
    ...
    void SDLApplication::SetFrameRate(double frameRate)
    {
        if (frameRate > 0)
        {
            ...
            // The predictive timeline automatically generates 32 new frame 
            // times whenever the period is updated.
            predictiveTimeline.setPeriod(RENDER_PERIOD_10NS);
        }
        else
        {
            UPDATE_PERIOD_10NS = 0;
            RENDER_PERIOD_10NS = 0;
            predictiveTimeline.reset();
        }
    }
    ...
    void SDLApplication::Init()
    {
        ...
        // Initialize the predictive frame timeline with the default render period
        predictiveTimeline.setPeriod(RENDER_PERIOD_10NS);
    ...
    void handleLinuxVsync(SDL_Window* sdlWindow, int64_t now10ns, int64_t lag, bool& shouldRender) {
        ...
        if (!shouldRender) {
            shouldRender = predictiveTimeline.consume(now10ns);
            if (shouldRender) {
                render_timestamp = now10ns - lastRenderTime;
                lastRenderTime = now10ns;
            }
...
#ifdef HX_WINDOWS
    {
...
#elif defined(HX_LINUX)
        {
            ...
            if (kbFocus && isVsyncRate()) {
                ...
                SDLWindow* focusedWindow = SDLWindow::windows[focusedWindowID];
                if (focusedWindow && focusedWindow->sdlWindow) {
                    handleLinuxVsync(focusedWindow->sdlWindow, now10ns, lag, shouldRender);
                } else {
                    shouldRender = predictiveTimeline.consume(now10ns);
                    if (shouldRender) {
                        render_timestamp = now10ns - lastRenderTime;
