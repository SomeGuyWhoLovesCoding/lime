...
namespace lime
{
    ...
    // ---------- Predictive Frame Time System ----------
    // Tracks frame timing using frame units internally, converting to/from
    // time units (10ns ticks) only when needed for sleep calculations or
    // external API boundaries.
    struct PredictiveFrameTimer {
        int64_t epochTime10ns = 0;           // Absolute time when frame 0 started
        int64_t predictedPeriod10ns = 0;      // Current predicted frame period in 10ns
        int currentFrame = 0;                 // Last completed frame number
        int targetFrame = 0;                  // Next frame we're waiting for
        bool initialized = false;
        
        // EMA state for period prediction (double for precision during accumulation)
        double periodEma = 0.0;
        static constexpr double EMA_ALPHA = 0.1;  // Smoothing factor
        
        void init(int64_t startTime10ns, int64_t initialPeriod10ns) {
            epochTime10ns = startTime10ns;
            predictedPeriod10ns = initialPeriod10ns;
            periodEma = (double)initialPeriod10ns;
            currentFrame = 0;
            targetFrame = 1;  // First frame to render is frame 1
            initialized = true;
        }
        
        void reset(int64_t startTime10ns, int64_t initialPeriod10ns) {
            initialized = false;
            init(startTime10ns, initialPeriod10ns);
        }
        
        // Update prediction based on actual measured period
        void updatePrediction(int64_t actualPeriod10ns) {
            if (actualPeriod10ns <= 0) return;
            
            periodEma = periodEma * (1.0 - EMA_ALPHA) + (double)actualPeriod10ns * EMA_ALPHA;
            
            // Clamp to prevent wild swings (50% to 200% of nominal)
            int64_t nominalPeriod = predictedPeriod10ns > 0 ? predictedPeriod10ns : RENDER_PERIOD_10NS;
            double minPeriod = (double)nominalPeriod * 0.5;
            double maxPeriod = (double)nominalPeriod * 2.0;
            
            if (periodEma < minPeriod) periodEma = minPeriod;
            if (periodEma > maxPeriod) periodEma = maxPeriod;
            
            predictedPeriod10ns = (int64_t)periodEma;
        }
        
        // Convert an absolute time to a frame number (may be fractional in concept, returns int)
        int timeToFrame(int64_t time10ns) const {
            if (!initialized || predictedPeriod10ns <= 0) return currentFrame;
            int64_t elapsed = time10ns - epochTime10ns;
            if (elapsed < 0) elapsed = 0;
            return currentFrame + (int)(elapsed / predictedPeriod10ns);
        }
        
        // Convert a frame number to absolute time
        int64_t frameToTime(int frame) const {
            if (!initialized) return epochTime10ns;
            return epochTime10ns + (int64_t)(frame - currentFrame) * predictedPeriod10ns;
        }
        
        // Check if we've reached or passed the target frame
        bool isTargetFrameReached(int64_t now10ns) const {
            return timeToFrame(now10ns) >= targetFrame;
        }
        
        // Get time until target frame (negative if already passed)
        int64_t timeUntilTargetFrame(int64_t now10ns) const {
            return frameToTime(targetFrame) - now10ns;
        }
        
        // Advance current frame to target and set next target
        void advanceToNextFrame() {
            currentFrame = targetFrame;
            targetFrame = currentFrame + 1;
        }
        
        // Handle vsync signal - updates prediction and resyncs frame counter
        void onVsyncSignal(int64_t vsyncTime10ns) {
            if (!initialized) {
                init(vsyncTime10ns, RENDER_PERIOD_10NS);
                return;
            }
            
            int64_t elapsedSinceEpoch = vsyncTime10ns - epochTime10ns;
            if (elapsedSinceEpoch <= 0) return;
            
            // Calculate how many frames should have passed
            int framesSinceEpoch = (int)(elapsedSinceEpoch / predictedPeriod10ns);
            int newCurrentFrame = framesSinceEpoch;
            
            // Only update if we're moving forward
            if (newCurrentFrame > currentFrame) {
                // Measure actual period from last frame
                int64_t actualPeriod = elapsedSinceEpoch / (int64_t)(newCurrentFrame - currentFrame);
                updatePrediction(actualPeriod);
                
                // Recalculate with updated period
                framesSinceEpoch = (int)(elapsedSinceEpoch / predictedPeriod10ns);
                currentFrame = framesSinceEpoch;
                targetFrame = currentFrame + 1;
            }
        }
        
        // Get the measured render timestamp (time of last frame in 10ns)
        int64_t getLastFrameTime(int64_t now10ns) const {
            return frameToTime(currentFrame);
        }
        
        // Get predicted period for external use
        int64_t getPredictedPeriod() const {
            return predictedPeriod10ns;
        }
    };
    
    static PredictiveFrameTimer frameTimer;
    ...
    static void choreographer_callback(long frameTimeNanos, void *data)
    {
        ...
        int64_t prevFrameTime = frameTimer.getLastFrameTime(frameTime10ns);
        render_timestamp = frameTime10ns - prevFrameTime;
        frameTimer.onVsyncSignal(frameTime10ns);
    }
#endif
    ...
    void SDLApplication::SetUncappedFrameRate(bool value)
    {
        ...
        if (value) {
            render_timestamp = 0;
            // Reset frame timer to start fresh in uncapped mode
            frameTimer.reset(getTime10ns(), RENDER_PERIOD_10NS);
        }
    ...
    void SDLApplication::SetFrameRate(double frameRate)
    {
        if (frameRate > 0)
        {
            ...
            // Update frame timer's predicted period
            if (frameTimer.initialized) {
                frameTimer.predictedPeriod10ns = RENDER_PERIOD_10NS;
                frameTimer.periodEma = (double)RENDER_PERIOD_10NS;
            }
        }
        else
        {
    ...
    void SDLApplication::SetRenderFrameRate(double renderFrameRate)
    {
        ...
        // Update frame timer's predicted period
        if (frameTimer.initialized && RENDER_PERIOD_10NS > 0) {
            frameTimer.predictedPeriod10ns = RENDER_PERIOD_10NS;
            frameTimer.periodEma = (double)RENDER_PERIOD_10NS;
        }
    }
    ...
    void SDLApplication::Init()
    {
        ...
        // Initialize the predictive frame timer
        frameTimer.init(getTime10ns(), RENDER_PERIOD_10NS);
    ...
    // Initialize the listener struct
    ...
    static void waylandFrameCallbackHandler(void* data, struct wl_callback* callback, uint32_t time) {
        ...
        int64_t now = getTime10ns();
        int64_t prevFrameTime = frameTimer.getLastFrameTime(now);
        render_timestamp = now - prevFrameTime;
        frameTimer.onVsyncSignal(now);
    ...
    void initWaylandVsync(SDL_Window* sdlWindow) {
        ...
        // Guard the SDL2 Wayland info access. 
        ...
        if (drmFd >= 0 && drmCrtcId != 0) {
            ...
            if (pollResult > 0 && (pfd.revents & POLLIN)) {
                ...
                if (VblankSequence != lastVBlankSeq) {
                    shouldRender = true;
                    int64_t prevFrameTime = frameTimer.getLastFrameTime(now10ns);
                    render_timestamp = now10ns - prevFrameTime;
                    frameTimer.onVsyncSignal(now10ns);
    ...
    // Uses frame units internally via PredictiveFrameTimer, converting to time
    // only when determining sleep targets.
    void handleLinuxVsync(SDL_Window* sdlWindow, int64_t now10ns, int64_t lag, bool& shouldRender) {
        shouldRender = false;
        ...
        if (cachedWaylandSurface) {
            if (waylandVsyncFired) {
                ...
                // Target is already advanced by onVsyncSignal, 
                // next frame time is implicit in the frame counter
            }
        } 
        ...
        // 3. Fallback predictive timer when no vsync is available
        if (!cachedWaylandSurface && drmFd < 0) {
            // Check if we've reached the predicted target frame
            if (frameTimer.isTargetFrameReached(now10ns)) {
                shouldRender = true;
                int64_t prevFrameTime = frameTimer.getLastFrameTime(now10ns);
                render_timestamp = now10ns - prevFrameTime;
                frameTimer.updatePrediction(render_timestamp > 0 ? render_timestamp : RENDER_PERIOD_10NS);
                frameTimer.advanceToNextFrame();
            }
        }
    }
    ...
    // --- Predictive Fallback Timer for non-Linux platforms ---
    // Uses frame units internally, converting to time only for sleep calculations.
    static void handlePredictiveFallbackTimer(int64_t now10ns, bool& shouldRender) {
        shouldRender = false;
        
        if (uncappedFramerate || RENDER_PERIOD_10NS == 0) {
            shouldRender = true;
            render_timestamp = 0;
            return;
        }
        ...
        if (frameTimer.isTargetFrameReached(now10ns)) {
            ...
            // Update prediction based on actual timing
            if (render_timestamp > 0) {
                frameTimer.updatePrediction(render_timestamp);
            }
    ...
    // --- Sleep until predicted next frame time ---
    // Converts frame units to time for the actual sleep call.
    static void sleepUntilNextFrame(int64_t now10ns) {
        if (uncappedFramerate || RENDER_PERIOD_10NS == 0) {
            return;
        }
        
        int64_t timeUntilTarget = frameTimer.timeUntilTargetFrame(now10ns);
        
        if (timeUntilTarget > minimalSleepCalc10ns) {
            // Sleep in chunks based on calculated minimal sleep time
            int64_t remaining = timeUntilTarget - minimalSleepCalc10ns;
            while (remaining > 0) {
                int64_t sleepChunk = (remaining < minimalSleepCalc10ns) ? remaining : minimalSleepCalc10ns;
                coolSleepUntil10ns(now10ns + timeUntilTarget - remaining);
                remaining -= sleepChunk;
                now10ns = getTime10ns();
                
                // Early exit if we've reached the target frame
                if (frameTimer.isTargetFrameReached(now10ns)) {
                    break;
                }
            }
    ...
    bool SDLApplication::Update()
    {
        ...
        // Poll inputs at the start of each iteration
        PollInputs();
        ...
        if (choreographer && shouldRenderFromCallback) {
            ...
            handlePredictiveFallbackTimer(now10ns, shouldRender);
        }
#elif defined(HX_LINUX)
        handleLinuxVsync(SDL_GetKeyboardFocus(), now10ns, lag, shouldRender);
#else
        handlePredictiveFallbackTimer(now10ns, shouldRender);
#endif
        ...
        {
            // Sleep until the predicted next frame time
            sleepUntilNextFrame(now10ns);
        }
