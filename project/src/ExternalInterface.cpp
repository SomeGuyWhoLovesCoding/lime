...
namespace lime {
	...
	*/
	...
	double miniaudio_getPlaybackPosition() {
		ma_uint64 pos = 0;
	...
	void miniaudio_seekToPCMFrame(int64_t pos) {
		/*ma_uint64 cursor;
	...
	void miniaudio_deactivate_decoder(ma_uint32 index) {
		if (index < g_decoderCount) {
			g_pDecodersActive[index] = MA_FALSE;
		}
	}

	void miniaudio_decoderVolumeKnob(ma_uint32 index, double volume) {
		g_pDecodersVolume[index] = volume;
	}

	void miniaudio_setPlaybackRate(float value) {
	...
	void miniaudio_start() {
		if (MIXER_STATE == 3) {
			miniaudio_seekToPCMFrame(0);
		}
	...
	ma_bool32 miniaudio_stopped() {
		return MIXER_STATE == 3;
	}

	void miniaudio_destroy() {
	...
	int miniaudio_loadFiles(value argv) {
	...
	//miniaudio shit
	DEFINE_PRIME0 (miniaudio_init); //zgf4
	DEFINE_PRIME1v (miniaudio_loadFiles); //zgf4
	DEFINE_PRIME0 (miniaudio_start);
	DEFINE_PRIME0 (miniaudio_stop);
	DEFINE_PRIME0 (miniaudio_destroy);
	DEFINE_PRIME2v (miniaudio_decoderVolumeKnob);
	DEFINE_PRIME1v (miniaudio_deactivate_decoder);
	DEFINE_PRIME1v (miniaudio_seekToPCMFrame);
	DEFINE_PRIME1v (miniaudio_setPlaybackRate);
